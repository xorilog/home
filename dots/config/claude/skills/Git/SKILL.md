...
1. **--signoff**: ALWAYS use the `--signoff` flag
   ...
   ```
   This adds: `Signed-off-by: Christophe Boucharlat <christophe.boucharlat@gmail.com>`
...
git config --global user.name "Christophe Boucharlat"
git config --global user.email "christophe.boucharlat@gmail.com"
```

**Note:** For Agregio repos (gitlab.com/agregio_group/*, github.com/agregio-solutions/*), the email is overridden via includeIf to `christophe.boucharlat@agregio-solutions.com`. The Signed-off-by line will automatically use the correct email based on the repo's remote URL.
